...
function tainacan_inventarios_get_collections_access_by_user() {
...
function tainacan_inventarios_get_control_collections_ids() {
...
function tainacan_inventarios_get_restrictive_ids($type = 'items') {
...
function tainacan_inventarios_get_allowed_users_id_cap($item) {
	$repositories_metadata = \tainacan_metadata();
...
function tainacan_inventarios_user_has_cap_filter( $allcaps, $caps, $args, $user ) {
	$exist_roles = !empty(array_intersect(tainacan_inventarios_get_restrictive_roles(), $user->roles));
	...
	if ( $exist_roles && is_array($args) && count($args) >= 3 ) {
		...
		if ( is_numeric( $entity_id ) ) {
			...
			if ( $item instanceof \Tainacan\Entities\Item && $item->get_status() != 'auto-draft') {
				$control_collections_ids = tainacan_inventarios_get_control_collections_ids();
				$col_id = $item->get_collection_id();

				$allowed_users_id = tainacan_inventarios_get_allowed_users_id_cap($item);
				$collections_access_by_user = tainacan_inventarios_get_collections_access_by_user();
...
add_filter( 'user_has_cap', 'tainacan_inventarios_user_has_cap_filter', 20, 4 );

function tainacan_inventarios_fetch_items_args($args, $user) {
	$exist_roles = !empty(array_intersect(tainacan_inventarios_get_restrictive_roles(), $user->roles));
	...
	if ($exist_roles && isset($post_type) && count($post_type) == 1 && \strpos($post_type[0], 'tnc_col_') === 0 ) {
		...
		if ( is_numeric($col_id) ){
			...
			$col_restrictive_ids = tainacan_inventarios_get_restrictive_ids('collections');
			...
			if ( in_array($col_id, $col_restrictive_ids) ) {
				...
				foreach($metadatas as $metadata) {
					...
					$items_id = tainacan_inventarios_get_restrictive_ids('items');
					if( $items_id === false ) {
						continue;
					}
					$args['meta_query'][] = [
...
function tainacan_inventarios_fetch_collections_args($args, $user) {
	if ( $args['posts_per_page'] == -1) {
		return $args;
	}

	$roles = $user->roles;
	$exist_restrictive_roles = !empty(array_intersect(tainacan_inventarios_get_restrictive_roles(), $roles));
	...
	$col_ids = tainacan_inventarios_get_collections_access_by_user();
...
function tainacan_inventarios_fetch_args($args, $type) {
	$user = \wp_get_current_user();
	
	if ( $type == 'items' ) {
		$args = tainacan_inventarios_fetch_items_args($args, $user);
	} elseif( $type == 'collections' ) {
		$args = tainacan_inventarios_fetch_collections_args($args, $user);
	}

	return $args;
}
add_filter( 'tainacan_fetch_args' , 'tainacan_inventarios_fetch_args', 10, 2 );
 ...
 * FORM HOOK
**/
add_action( 'tainacan-register-admin-hooks', 'tainacan_inventarios_register_admin_hooks');
function tainacan_inventarios_register_admin_hooks()
{
	if ( function_exists( 'tainacan_register_admin_hook' ) )
	{
		tainacan_register_admin_hook( 'metadatum', 'tainacan_inventarios_set_user_to_restrict_access_items_form', 'end-left', [ 'attribute' => 'metadata_type', 'value' => 'Tainacan\Metadata_Types\User' ] );
		tainacan_register_admin_hook( 'role', 'tainacan_inventarios_set_role_to_restrict_access_items_form', 'end-right' );
	}
}

function tainacan_inventarios_set_user_to_restrict_access_items_form()
{
	...
	?>
		...
		<div class="tainacan-set-user-to-restrict-access"> 
			...
			<div
				...
				<label class="label">
					<?php _e('Restringir acesso dos usuários', 'tainacan-inventarios'); ?>
					<span class="help-wrapper">
						...
						<div class="help-tooltip">
							<div class="help-tooltip-header">
								<h5 class="has-text-color"><?php _e('Restringir acesso', 'tainacan-inventarios'); ?></h5>
							</div>
							<div class="help-tooltip-body">
								<p><?php _e('Ao ativar a restrição de acessos os usuários listados nesse metadado terão seu acesso aos itens restringido.', 'tainacan-inventarios'); ?></p>
							</div>
						</div>
					</span>
				</label>
...
\add_filter( 'tainacan-api-response-metadatum-meta', 'tainacan_inventarios_set_user_to_restrict_access_items_add_meta_to_response', 10, 2 );
function tainacan_inventarios_set_user_to_restrict_access_items_add_meta_to_response( $extra_meta, $request )
{
	$extra_meta = array_merge($extra_meta, array('set_user_to_restrict_access'));
	return $extra_meta;
}

\add_action( 'tainacan-insert-tainacan-metadatum', 'tainacan_inventarios_set_user_to_restrict_access_items_create', 10, 2 );
function tainacan_inventarios_set_user_to_restrict_access_items_create($metadatum) {
...
function tainacan_inventarios_set_role_to_restrict_access_items_form()
{
	$repositories_collections = \tainacan_collections();
	...
	?>
		<div class="name-edition-box tainacan-set-role-to-restrict-access">
			<label for="set_role_to_restrict_access"><?php _e('Restringir edição dos itens baseado no relacionamento e equipe', 'tainacan-inventarios'); ?></label>
			<select name="set_role_to_restrict_access" id="set-user-to-restrict-access-select">
				<option value="yes"><?php _e('Sim', 'tainacan-inventarios'); ?></option>
				<option value="no"><?php _e('Não', 'tainacan-inventarios'); ?></option>
			</select>
			<p><span class="dashicons dashicons-info"></span>&nbsp;<?php _e('Se um usuário desta função/papel estiver listado no metadado tipo usuário ("equipe") com a configuração "Restringir acesso aos usuários" de uma coleção A que também tiver um metadado de relacionamento com uma coleção B, este usuário somente poderá editar itens na coleção B, onde o metadado de relacionamento com a A tiver como valor coleção onde o usuário faz parte da equipe.', 'tainacan-inventarios'); ?></p>
		</div>
		<br>
...
\add_action( 'tainacan-api-role-prepare-for-response', 'tainacan_inventarios_set_role_to_restrict_access_items_create', 10, 2 );
function tainacan_inventarios_set_role_to_restrict_access_items_create($role, $request) {
